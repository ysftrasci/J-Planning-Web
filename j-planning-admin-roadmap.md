...
3. **[✅ TAMAMLANDI] Askıya alınan kullanıcı anlık atılmıyor:** Admin bir kullanıcıyı askıya aldığında, kullanıcı halihazırda açık bir oturuma sahipse hemen atılmıyordu. Düzeltildi (Aşama 27): Control Plane asıl referans (source of truth) olarak kalıyor; admin askıya aldığında worker önce Control Plane'i güncelliyor, sonra Firestore'a `users/{uid}.isDisabled` alanı yazılıyor (sadece bu ve `disabledAt`/`updatedAt` alanlarına admin yazabilecek şekilde Firestore rules'da alan bazlı kısıtlama uygulandı). İstemci tarafında `AuthContext.jsx` bu alanı `onSnapshot` ile gerçek zamanlı dinliyor, değişikliği yakalayınca anında `signOut()` çalıştırıp `/login`'e yönlendiriyor; ayrıca worker'dan 403 `ACCOUNT_DISABLED` gelmesi ve sekme odağa gelmesi durumları için fallback kontrolleri var. Hem bilgisayarda hem telefonda canlı ortamda test edildi, açık sekme askıya alınır alınmaz kendiliğinden atıldı.
4. **[✅ TAMAMLANDI] Görev silme:** Faz 4'te bilinçli olarak whitelist dışı bırakılmıştı. Eklendi (Aşama 28): `DELETE /admin/users/:uid/tasks/:taskId` endpoint'i — admin claim kontrolü, atanan (`RECEIVED`) görevlerin silinememesi koruması, `tasks` + `task_records` kalıcı silme, Control Plane `task_count` senkronu, ve silinen görevin tam içeriğiyle `DELETE_TASK` audit log kaydı. Frontend'de onay penceresi ve atanan görevlerde devre dışı buton var. Hem admin panelde hem gerçek uygulamada (farklı cihazdan) kalıcı silindiği doğrulandı, atanan görev koruması ve audit log kaydı test edildi.
5. **[✅ TAMAMLANDI] Kullanıcı kodu değiştirme:** Firebase `uid` değil, `userCodes` koleksiyonundaki görünen kod (örn. `JP-L73BEV`) admin panelden değiştirilebilir hale getirildi (Aşama 29). Worker'a `PATCH /admin/users/:uid/code` eklendi (format doğrulaması + `UPDATE_USER_CODE` audit log). Frontend, Firestore `writeBatch` ile atomik güncelleme yapıyor (yeni `userCodes/{newCode}` oluştur + eski `userCodes/{oldCode}` sil + `users/{uid}.userCode` güncelle — üçü birden başarılı olur ya da hiçbiri yazılmaz). Çakışma koruması (409, kod başka kullanıcıda kullanılıyorsa reddediliyor) ve eski kodun gerçekten serbest kalıp başka bir hesaba atanabildiği canlıda doğrulandı.
6. **[✅ TAMAMLANDI] E-posta doğrulama akışı tutarsızlığı:** "Doğruladım, devam et" butonu çalışmıyordu. Kök neden bulundu (Aşama 30): `refreshAuthUser` fonksiyonu `AuthContext.jsx`'te çağrılıyordu ama hiç tanımlanmamıştı, bu yüzden buton `TypeError` ile sessizce başarısız oluyordu. Eklendi: `refreshAuthUser` (Firebase'den taze `emailVerified` durumu çeker) + sekmeye geri dönüldüğünde otomatik algılama (`focus` dinleyicisi, askıya alma dinleyicisinden izole). Ayrıca "Tekrar Gönder" butonunun Firebase rate-limit'ine (400 `TOO_MANY_ATTEMPTS_TRY_LATER`) takılması için `localStorage` tabanlı kalıcı 60 saniyelik cooldown eklendi (Aşama 30.1) — sayfa yenilense bile sıfırlanmıyor. Test sırasında görülen ayrı bir `reportAllChanges`/`startTime` konsol hatası, tarayıcı eklentisi kaynaklı olduğu doğrulanıp (eklentiler kapatılınca hata kayboldu) kapatıldı, kod tabanıyla ilgisi yok.
7. **[Yeni Bulundu, Sırada] Kayıt sırasında girilen ismin tutarsız görünmesi:** Kayıt formunda girilen isim, bazen profil sayfasında ve admin panelde doğru görünüyor, bazen "Kullanıcı1" gibi bir varsayılan veya e-postanın "@" öncesi kısmı görünüyor — aynı akış farklı denemelerde farklı sonuç veriyor (race condition şüphesi). Henüz kök nedeni araştırılmadı.
